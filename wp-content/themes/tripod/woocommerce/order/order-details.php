...
?>
<h2 class="order-details"><?php _e( 'Order Details', 'woocommerce' ); ?></h2>
			...
			foreach($order->get_items() as $item) {
				...
				if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
					...
					foreach ( $download_file_urls as $file_url => $download_file_url ) {
						...
						$links[] = '<small><a href="' . $download_file_url . '" class="woo-download-link icon-download">' . sprintf( __( 'Download file%s', 'woocommerce' ), ( count( $download_file_urls ) > 1 ? ' ' . ( $i + 1 ) . ': ' : ': ' ) ) . $filename . '</a></small>';
...
<div class="col2-set addresses woo-address">
