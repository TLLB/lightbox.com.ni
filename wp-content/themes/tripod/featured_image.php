...
<?php if ( !post_password_required() ) { ?>
    <div class="featimg <?php echo $cropp_img_class. ' '; if(get_post_format( $post -> ID ) == 'video'){ echo 'video-post'; } ?>">
        <?php 
        if (options::logic( 'blog_post' , 'enb_cropp_img' ) && get_post_format( $post -> ID ) != 'gallery' && get_post_type() != 'page' ) { 
            wp_localize_script('functions', 'featmask_enb', true);
            ?>
        <div class="featmask"></div>
        <?php } else {
            wp_localize_script('functions', 'featmask_enb', false);
