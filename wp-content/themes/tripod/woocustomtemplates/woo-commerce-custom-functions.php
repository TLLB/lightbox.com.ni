<?php
    ...
    function woo_styles_enabled_warning(){
    	/*  check if woocommerce is installed*/
	    if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) && get_option( 'woocommerce_frontend_css' ) == 'yes' 

	    	&& (!isset( $_COOKIE[ ZIP_NAME . "woo_styles" ] ) || ( isset( $_COOKIE[ ZIP_NAME . "woo_styles" ] ) && $_COOKIE[ ZIP_NAME . "woo_styles" ] != 'closed' )) ) {
	?>
			
            <div id="hide-this" class="error">
            	<a class="icon-close" onclick="jQuery( this ).parent().hide()" href="javascript:void(0);"></a>
                <div class="message"> 
                	<?php
                		echo sprintf(__('In order to have a better experience, please deactivate Woocommerce default styles from <a href="%s">here</a> ','cosmotheme'),home_url().'/wp-admin/admin.php?page=woocommerce_settings');
                	?>
                	
                	<img  alt="" src="<?php echo get_template_directory_uri()?>/images/setting.jpg" />
                </div>
            </div>
